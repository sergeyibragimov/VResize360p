...
async def hd_generate(
	...
	try:
		...
		if not scales:
			scales.append("x".join(map(str, (from_w, from_h))))
...
async def ar_generate(
	...
	for la in filter(lambda x: x, tuple(list_ar)):
		...
		try:
			...
			if all(
				(from_w, from_h)
				) and not (from_w, from_h) in scales:
				scales.append((from_w, from_h))  # default_for_any

	# width*height / aspect_ratio
	try:
		scales_result: list[str] = list(
			set(
				[
					"x".join(map(str, (f, s, round(f / s, 3))))
					for f, s in scales
					if all((f, s))
				]
			)
		)  # str_list(width/height/ar) # python 3.9
	except BaseException as e:  # if_error
		scales_result = []
		logging.error(
...
async def combine_br():  # abr + vbr(combine_equal_files)
	...
	if all(
		(vbr_dict, abr_dict, somebase_dict)
	):
		br_dict: dict[str, list] = {
			k: [v, v2]
			for k, v in vbr_dict.items()
			for k2, v2 in abr_dict.items()
			if all((k, k2, k == k2)) and os.path.exists(k)
		}  # len(v3.split(":")) > 1 # python 3.9
	elif all(
		(vbr_dict, abr_dict, not somebase_dict)
	):
		br_dict: dict[str, list] = {
			k: [v, v2, 0, 0]
			for k, v in vbr_dict.items()
	...
	if all(
		(br_dict, somebase_dict)
	):  # new_jobs # +vbr/abr
		bitrate_dict: dict[str, str] = {
		...
		}  # check_exists # with_optimized_jobs(new) # python 3.9
	elif all(
		(br_dict, not somebase_dict)
		):  # default # +vbr/abr
		bitrate_dict: dict[str, str] = {
			k.strip(): 'cmd /c c:\\downloads\\mytemp\\ffmpeg -hide_banner -y -i "%s" -preset medium -threads 2 -c:v libx264 -b:v %dK -maxrate %dK -bufsize %dK -threads 2 -c:a aac -b:a %dK "%s"'
			% (
		...
		}  # check_exists	# without_optimized_jobs(no_jobs) # python 3.9
...
async def read_br():  #
	"""read_biitrate_base_for_equal_files"""
	try:
...
with open(
	"c:\\downloads\\mytemp\\overload.csv", "a", newline="", buffering=1
) as ocf:  # buffering
...
async def calc_number_of_day(
	...
	if all(
		(sleep_if, dt_str == sleep_if, not is_default)
	):
		sleep(2)
...
def unixtime_to_date(unixtime: float = 0.0) -> str:  # os.path.getmtime(file)
...
async def redate_files(lst: list = []):
	...
	if len(lst) > 1: # sorted_by_modify_date
		...
		try:
			create_and_old_time_list: list[int] = [
				i
...
async def gcd_from_numbers(lst: list = []) -> list:
	...
	if all(
		(min(lst) > 0, min(lst) < max(lst))
	):  # convert_to_generator(refactor)
...
# is_period: list[bool] = [False, False, False] # python 3.9
def ff_to_days(
	ff: str = "",
	period: int = 30,
	is_dir: bool = False,
	...
	try:
		...
		fdate = os.path.getctime(ff)  # unixdate(file/folder) # getmtime -> debug(getctime/optimal) # dir /t:w
		...
		try:
			...
			if period >= 0 and is_period.count(False) == len(
				...
				period_filter_list = []
				period, is_new_period = 0, False

				# week / month / year
				period_filter_list = [
					abs(today - ndate).days // 7,
					abs(today - ndate).days // 30,
					abs(today - ndate).days // 365,
				]

				period, is_new_period = max(period_filter_list), (
					period_filter_list.count(True) > 0
				)

				if abs(today - ndate).days <= 2:  # append_every_day
					with open(
						twoday_base, "a", encoding="utf-8", buffering=1
					) as tdf:  # buffering
						tdf.writelines("%s\n" % ff.split("\\")[-1])
...
async def folders_from_path(
	...
	try:
		folder_scan_full: list[str] = list(
			set(
				[
	...
	folder_scan_full.sort(key=os.path.getctime)  # reverse=False # sort_by_abc # getmtime -> debug(getctime)
	...
	try:
		...
		if all(
			...
			desc_dict |= desc_dict_filter  # python 3.9 
			# desc_dict.update(desc_dict_filter)
			# desc_new = {**desc_dict, **desc_dict_filter} # a.update(b) # a |= b # python 3.9
	...
	try:
		...
		# folder_scan = (
		# folder_scan[0:1000] if len(folder_scan) > 1000 else folder_scan
		# ) # limit_folders_for_scan

		if all(
			(folder_scan, lang_list, is_tvseries)
		):  # folder_list(reverse) / folder_list_by_lang # tvseries_only
			if is_rus:
				folder_scan_rus: list[str] = [
	...
	try:
		...
		with open(days_base, "a", encoding="utf-8", buffering=1) as dbf:  # buffering
	...
	try:
		...
		with open(month_base, "a", encoding="utf-8", buffering=1) as mbf:  # buffering
	...
	try:
		...
		with open(year_base, "a", encoding="utf-8", buffering=1) as ybf:  # buffering
	...
	for fsf in filter(lambda x: os.path.isdir(x), tuple(folder_scan_full)):
		...
		fsizes: list[int] = [os.path.getsize(os.path.join(fsf, f)) for f in os.listdir(fsf)]  # python 3.9
		files: list[str] = [
			os.path.join(fsf, f)
		...
		if all((len_desc > 0, len_files > 0)):  # default_count(skip_null)
			...
			# logging.info("папка %s, %d описаний, %d файлов" % (fsf.split("\\")[-1], len_desc, len_files))
	...
	def avg_calc(s, l, /):  # s, l, *(pos_or_keyw) # s, l, /(pos) # python 3.8
		try:
			ag = lambda s, l: s // l
			assert l, ""
	...
	year_count: dict[str, list] = {
		k: v for k, v in all_period_dict.items() if any(("год" in k, "лет" in k))
	}  # python 3.9

	# year_ago_dict |= year_count  # python 3.9
	# year_ago_dict.update(year_count) 
	# year_new = {**year_ago_dict, **year_count} # a.update(b) # a |= b # python 3.9
	...
	month_count: dict[str, list] = {
		k: v for k, v in all_period_dict.items() if "месяц" in k
	}  # python 3.9

	# month_ago_dict |= month_count  # python 3.9
	# month_ago_dict.update(month_count) 
	# month_new = {**month_ago_dict, **month_count} # a.update(b) # a |= b # python 3.9
	...
	week_count: dict[str, list] = {
		k: v for k, v in all_period_dict.items() if "недел" in k
	}  # python 3.9

	# days_ago_dict |= week_count  # python 3.9
	# days_ago_dict.update(week_count) # update_current_by_week 
	# days_new = {**days_ago_dict, **week_count} # a.update(b) # a |= b # python 3.9
	...
	for fsf in filter(lambda x: os.path.isdir(x), tuple(folder_scan_full)):
		...
		if not fsf.strip() in fsf_set:
			fsf_set.add(fsf.strip())  # add_if_not_runned # discord
		else:
		...
		try:
			...
			)  # desc(1) / files(0/1) # python 3.9
			is_not_found: tuple[bool] = (
				len(list(filter(lambda x: "mp4" in x, tuple(list_files)))) == 0
				and len(list(filter(lambda x: "txt" in x, tuple(list_files)))) == 0
			)  # desc(0) / files(0) # python 3.9
			is_two_desc: tuple[bool] = (
				len(list(filter(lambda x: "mp4" in x, tuple(list_files)))) >= 0
				and len(list(filter(lambda x: "txt" in x, tuple(list_files)))) >= 2
			)  # desc(2) / files(0/1) # python 3.9
			...
			for dl in tuple(dlist):  # only_first_line # debug
				...
				try:
					...
					if (
						...
						len_and_codepage.add(len_and_codepage_str)  # discord
				...
				if desc_regex.findall(dl):
					#parse_list: list[str] = [] # python 3.9
				...
				if result_days:
					...
					try:
						...
						if result_temp != result_days:
							result_temp |= result_days  # python 3.9 
							# result_temp.update(result_days)
							# result_temp_new = {**result_temp, **result_days} # a.update(b) # a |= b # python 3.9
				...
				if result_month:
					...
					try:
						...
						if result_temp2 != result_month:
							result_temp2 |= result_month  # python 3.9
							# result_temp2.update(result_month)  
							# result_temp_new2 = {**result_temp2, **result_month} # a.update(b) # a |= b # python 3.9
				...
				if result_year:
					...
					try:
						...
						if result_temp3 != result_year:
							result_temp3 |= result_year  # python 3.9
							# result_temp3.update(result_year)  
							# result_temp_new3 = {**result_temp3, **result_year} # a.update(b) # a |= b # python 3.9
			...
			if all(
				...
				if is_found:  # desc(0/1), files(1) # some_found
					...
					if all((len(desc_list) == 1, is_backup.count(True))):
						...
						try:
							soundtrack_filter: dict[str, str] = {
								t.strip(): sl.strip()
								for sl in filter(
									lambda x: any(
										(
								...
								and tmp
							}  # t.replace(sl, "*" * len(sl)).strip() # descrypt/debug # python 3.9
						except:
							soundtrack_filter = {}
							is_error = True
						else:
						...
						soundtrack_filter_top: dict[str, str] = {
							k: v
							for k, v in soundtrack_filter.items()
							if all((k, not k in [*soundtrack_dict]))
						}  # if_new_soundtrack(filter_current_files) # python 3.9
						...
						if soundtrack_filter_top:
							...
							soundtrack_filter |= soundtrack_filter_top  # python 3.9
							# soundtrack_filter.update(soundtrack_filter_top)  
							# soundtrack_filter_new = {**soundtrack_filter, **soundtrack_filter_top} # a.update(b) # a |= b # python 3.9
						...
						if all(
							...
							soundtrack_dict |= soundtrack_filter  # python 3.9
							# soundtrack_dict.update(soundtrack_filter)  # update_base_from_filter 
							# soundtrack_dict_new = {**soundtrack_dict, **soundtrack_filter} # a.update(b) # a |= b # python 3.9
							...
							try:
								soundtrack_count: dict[str, list] = [
									(
										v.strip(),
							...
							try:
								...
								if soundtrack_count:
									...
									soundtrack_count_list: list[tuple] = [
									...
									try:
										soundtrack_count_new: dict[str, int] = {
											k: int(v)
										...
										a = s / l
										class_dict: dict[str, float] = {
											k: round((v / s) * 100, 2)
											for k, v in soundtrack_count_new.items()
											if v - a > 0
										}  # 0..100%(popular_classify) # python 3.9
...
async def clear_segments(lst: list = []):
	...
	try:
		seg1: list[str] = [fn.split(".")[0] for fp, fn in os.path.split(files)]  # segments
		seg2: list[str] = [fn.split(".")[0] for fp, fn in os.path.split(files2)]  # files
...
try:
	temp: list[str] = list(set([al.strip() for al in tuple(all_list) if al]))  # python 3.9
except:
	temp = []
finally:
...
filter_top_list: list[str] = [al.strip() for al in tuple(all_list) if al]  # python 3.9 # shorts
...
if filter_top_list:
	...
	filter_top_by_folders: list[str] = list(
		set(
			[
				ftl.strip()
	...
	filter_for_new_backup: list[str] = list(
		set(
			[
				k.strip()
				for k in [*somebase_dict]
	...
	# filter_for_new_backup = list(set(
	# [k.strip() for k in [*somebase_dict] if ff_to_days(ff=k, period=12*days, is_dir=False, is_less=True, is_any=False)[0] != None]
	# )) # 12_year_and_less # type2 # for k, v in somebase_dict.items()
...
def write_log(
	...
	if all((desc != None, txt != None, isinstance(desc, str), isinstance(txt, str))):
		...
		if all((txt.strip(), is_logging)):  # if not is_error
			# logging.info(";".join([desc.strip(), txt.strip()])) # logging
			logging.info(
				" ".join(
					[desc.strip().replace("debug ", "@"),
					txt.strip()
				])
			)  # logging # debug
	...
	if lprint:
		...
		for lp in filter(lambda x: x, tuple(lprint)):
			...
			try:
				assert lp in check_log, ""
			except AssertionError:
				check_log.add(lp)  # discord
...
def log_error(f):
	"""decorator # check_function_by_error(logging)"""
	...
	if all((folder, fname, m3u8_file)):
		...
		if is_run:
			try:
				...
				if p == 0:
					if not is_stay and os.path.exists(
						m3u8_file
			...
			if not is_stay and os.path.exists(m3u8_file):  # delete_if_stay_false
				os.remove(m3u8_file)
...
async def find_mac(mac_address: str = ""):
	...
	every_first_day/every_5day/every_optimal_end_month(update_if_no_job_time)
	if any((dt.day == 1, dt.day % 5 == 0, all((dt.day == dt_day, dt_day)))) and any((dt.hour < 9, dt.hour > 18)):
	"""
	try:
...
async def ip_to_mac(ip: str = "") -> tuple:
	"""single_by_async"""
...
try:
	...
	if acl:
		...
		with open(
			squidr_base, "a", encoding="utf-8", buffering=1
		) as saf:  # append(is_unique) # buffering
...
async def hostname_and_ip() -> tuple:
	"""get_hostname_and_ip"""
...
async def current_ip() -> tuple:
	"""get_ip_by_current_lan"""
...
# @log_error
async def kill_proc_by_name(proc_name: str = "", kill: bool = True):  # hide # *(pos_or_keyw), ... # ..., /(pos) # python 3.8
	"""
	...
	try:
		for proc in psutil.process_iter():
			...
			if proc.name == PROCNAME:
				...
				if all((not "SYSTEM" in p.username, kill)):
...
async def skip_run_date():
	...
	try:
		holiday_status = holidays[".".join(map(str, (day, month)))]
	except:
...
if (
	all((is_status.count(True) > 0, not srd))
):
	...
	if (
		any((ut <= ctme.hour < mytime["sleeptime"][1], ctme.hour < mytime["sleeptime"][1]))
	):  # debug # 5 <= x < 8
...
async def battery_info():
	...
	try:
		...
		if plugged:
			...
			if (
				all((percent != None, isinstance(int(percent), int)))
			):  # float -> int
...
def mdate_by_days(
	...
	# debug_for_folder
	today = datetime.today()  # datetime
	fdate = os.path.getctime(filename)  # unixdate(file/folder) # getmtime -> debug(getctime)
	ndate = datetime.fromtimestamp(fdate)  # datetime
	...
	try:
		...
		elif all((is_any, days_ago >= 0, period >= 0)):
			write_log(
				"debug file[dayago]",
...
# @log_error
async def days_by_list(lst: list = [], is_avg: bool = False):  # 8 # hide # *(pos_or_keyw), ... # ..., /(pos) # python 3.8
	try:
		assert lst and isinstance(
			lst, list
	...
	with unique_semaphore:
		for l in tuple(lst):
			...
			try:
				fdate = os.path.getctime(l)  # unixdate # getmtime -> debug(getctime/optimal)
				ndate = datetime.fromtimestamp(fdate)  # datetime
			except:
				continue
			else:
...
# @log_error
def fspace(src: str = "", dst: str = "", is_Log: bool = False) -> bool:  # 11 # hide # *(pos_or_keyw), ... # ..., /(pos) # python 3.8
	try:
...
# @log_error
def MyNotify(txt: str = "", icon: str = "", sec: int = 10):  # 19 # hide # *(pos_or_keyw), ... # ..., /(pos) # python 3.8
	error: bool = False
...
# @log_error
def walk(dr: str = "", files_template: str = ""):  # hide # *(pos_or_keyw), ... # python 3.8
...
class Get_AR:
	...
	def width_to_ar(
		...
		if all((width > owidth, owidth)):  # if_need_optimal_ar # with_change
	...
	def height_to_ar(
		...
		if all((height > oheight, oheight)):  # with_change
...
class MyMeta:
	...
	def get_width_height(
		...
		try:
			if "x" in width_height_str:
				width, height = map(
					int,
					(
						int(width_height_str.split("x")[0]),
						int(width_height_str.split("x")[-1]),
					),
				)
		...
		try:
			...
			if all(
				...
				try:
					ow, oh = map(int, (int(ga.width), int(ga.height)))
				except:
					ow = oh = 0
	...
	def calc_vbr(
		...
		# old_vbr
		# motion = (
		# 4 if all((height > 720, height)) else 2
		# )  # motion={Middle motion:2, High motion:4} # baseline(1), main(2), high(4) # look_like_profile
		...
		if any((owidth != width, owidth / oheight != width / height)) and all(
			...
			if all((not fname in fnames_set, fname)):
				fnames_set.add(fname)  # discord

			if not filename in filenames_set:
				filenames_set.add(filename)  # discord

				if all((not nscale in scales_set, nscale)):
					scales_set.add(nscale)  # new_scale(logging) # discord
		...
		if any((oheight != height, owidth / oheight != width / height)) and all(
			...
			if all((not nscale2 in scales_set, nscale2)):
				scales_set.add(nscale2)  # new_scale(logging) # discord
		...
		try:
			...
			if all((fsize, gl)):
				...
				tmp: list[int] = list(set([
					vl for vl in filter(lambda x: x, tuple(vbr_list))
				]))  # python 3.9
	...
	def get_frame_quality(self, filename, is_log: bool = True) -> float:  # 2
		...
		"""
		UHD (4K);2160p;3840;2160;60;23000-29500
		UHD (4K);2160p;3840;2160;30;14000-18200
		2K;1440p;2560;1440;60;?
		2K;1440p;2560;1440;30;?
		FullHD;1080p;1920;1080;60;5800-7400
		FullHD;1080p;1920;1080;30;4500-5300
		HD;720p;1280;720;60;3500-4400
		HD;720p;1280;720;30;2500-3200
		SD;480p;854;480;30;1250-1600
		SD;360p;640;360;30;700-900
		SD;240p;426;240;30;400-600
		"""
...
async def folders_filter(
	...
	if full_folder:
		tmp: list[str] = list(set([
			ff.strip() for ff in filter(lambda x: x, tuple(full_folder))
		]))  # python 3.9

		full_folder = sorted(tmp, key=os.path.getctime)  # reverse=False # getmtime -> debug(getctime/optimal)
			...
			if files:
				...
				try:
					...
					if all((len(ff_dict) >= 0, ff_last)):  # null_or_some # last_files
						ff_dict |= ff_last  # python 3.9
						# ff_dict.update(ff_last) 
						# ff_dict_new = {**ff_dict, **ff_last} # a.update(b) # a |= b # python 3.9
		...
		try:
			if full_folder2:
				...
				if all(
					...
					}  # stay_not_optimized_jobs_by_current_month # +getmtime
			...
			if os.path.exists(l):
				if all(
					...
					if all((not new_file in job_file_set, new_file)):
						...
						job_file_set.add(new_file)  # discord
						...
						if all(
							...
							try:
								...
								]  # video_regex.findall(ld) # python 3.9
							except BaseException as e:
								scan_files: list = []
								write_log(
		...
		if fspace(file1, file2):  # reserve(dspace)(ok)
			if not equal_fsize:
				...
				if any((file1[0] < file2[0], file2[0] >= file1[0])) and file1 != file2:
					...
					# os.remove(file1)
		# elif not fspace(file1, file2): # reserve(dspace)(bad)
...
async def seasonvar_parse(
	...
	async def soundtrack_save():  # 2
		...
		try:
			# soundtrack_list = soundtrack[::-1] # sorted(soundtrack, reverse=True) # cba_by_index
	...
	try:
		...
		if all((video_ext_regex.findall(filename), is_need_change)):
...
async def load_timing_from_xml(
	...
	if timing:  # logging_if_some_data # ... Xml loaded # dict's_to_list
		...
		hours, minutes = map(int, (int(timing[0]["hh"]), int(timing[0]["mm"])))
...
async def save_timing_to_xml(hours: int = 0, minutes: int = 0):  # 3
	"""get_time_if_have_ready_jobs_else_null_time"""
...
async def project_done(
	...
	for pf in filter(lambda x: os.path.exists(x), tuple(proj_files)):  # new(yes_gen)
		...
		if ff_last:
			...
			# len_file_list = [
			# {"file": dl, "length": MM.get_length(dl)} for dl in filter(lambda x: os.path.exists(x), tuple(dub_list)) if dl
			# ]
	...
	if all((proj_files, datelist)):  # need_automatic_clean
		...
		with unique_semaphore:
			for dl in datelist:
				...
				if all((gl1, gl2)) and os.path.exists(fullname):
		...
		backupbase_dict |= backupbase_filter_dict  # python 3.9
		# backupbase_dict.update(backupbase_filter_dict)  # update_backup_jobs_by_filter 
		# backupbase_dict_new = {**backupbase_dict, **backupbase_filter_dict} # a.update(b) # a |= b # python 3.9
		...
		if len(proj_files) >= 0:
			...
			# temp = [
			# (False, True)[os.path.exists(pf)] for pf in proj_files
			# ] # ternary
				...
				if (
					...
					try:
						if not parsefile.split("$")[0] in moved_list:
							moved_list.add(parsefile.split("$")[0])  # discord
					except BaseException as e:
						write_log(
...
async def update_bigcinema():  # 11
	...
	for bc in filter(lambda x: x, tuple(big_cinema)):
		...
		maxdate_m = max(
			files, key=os.path.getmtime
		)  # 'C:\\Python27\\LICENSE.txt' # modify # +getmtime

		try:
			maxdate_file = os.path.getmtime(maxdate_m)  # modify # debug # pass_2_of_2 # +getmtime
		except:
			maxdate_file = None
	...
	try:
		...
		for f in fl:
			...
			if isinstance(f, list):
				if len(f) == 1:
					skip_set.add(f[0])  # discord
				elif len(f) > 1:
					skip_set.add("_".join(f))  # discord
...
async def hh_mm_ss(length: int = 0) -> str:  # 5
...
async def filter_from_list(lst: list = []) -> list:  # files -> current_files + base #5
	...
	try:
		# @python 3.9
		# short_list: list[str] = [crop_filename_regex.sub("", sm.split("\\")[-1]).strip() for sm in filter(lambda x: os.path.exists(x), tuple(some_files))] # equal
		# short_list: list[str] = [crop_filename_regex.sub("", sm.split("\\")[-1]).split("_")[0].strip() if sm.split("\\")[-1].count("_") > 0 else crop_filename_regex.sub("", sm.split("\\")[-1]).strip() for sm in filter(lambda x: os.path.exists(x), tuple(some_files))]  # match_or_equal # only_first
...
if __name__ == "__main__":  # debug/test(need_pool/thread/multiprocessing/queue)
	...
	try:
		...
		for maf in filter(lambda x: x, tuple(my_arg_filter)):
			if maf.count("_") > 0:
				...
				filter_set3.add(maf)  # no_delims_but_not_sort # discord
	...
	if any((filter1, filter2, filter3, filter4)):  # filter5
		...
		try:
			...
			if short_files:
				...
				# a = sorted(a, key=len, reverse=False) # ['3', '11', '44', '222'] # abc_by_values
				# a = sorted(a, key=len, reverse=True) # ['222', '11', '44', '3'] # cbc_by_values

				# a = sorted(a, reverse=False) # ['11', '222', '3', '44'] # abc_by_string
				# a = a[::-1] # sorted(a, reverse=True) # ['44', '3', '222', '11'] # cbc_by_string
				...
				if sfl:
					try:
						short_files = list(short_files_gen())  # new(yes_gen)
						# short_files = [
						# s.strip() for s in filter(lambda x: x, tuple(sfl))
						# ]
					except:
						short_files = []
		...
		filter_list = (
			list(set(short_list)) if short_list else []
		)  # is_no_lambda
		...
		with unique_semaphore:
			for lf in filter(lambda x: os.path.exists(x), tuple(lfiles)):
				...
				if all((not fname in unique, fname)) and os.path.exists(lf):
					unique.add(fname)  # short_file(first)_by_set # discord
					full_list.add(lf)  # full_filename(first)_by_set # discord
	...
	if (
		...
		# [
		# "\\".join(["\\".join(ltf.split("\\")[:-1]), ltf.split("\\")[-1]]) for ltf in lt
		# ] # ['c:\\downloads\\mytemp\\hello.file']
	...
	if lfiles_total:
		...
		# tmp = list(set(
		# [lt.strip() for lt in filter(lambda x: x, tuple(lfiles_total))]
		# )) # new(yes_gen)
	...
	with unique_semaphore:
		for lf in filter(lambda x: x, tuple(lfiles)):
			...
			if all((not fname in short_set, fname)):
				short_set.add(fname)  # discord
				full_list.add(lf)  # discord
		...
		if filecmdbase_dict:  # is_optimize_and_run_by_time
			...
			for k, v in filecmdbase_dict.items():
				...
				if not k in full_set:
					full_set.add(k.strip())  # save_unique_job # discord
	...
	try:
		...
		if listfiles_dict:
			pathbase_dict |= listfiles_dict  # python 3.9
			# pathbase_dict.update(listfiles_dict)  # update_files(dict) 
			# pathbase_dict_new = {**pathbase_dict, **listfiles_dict} # a.update(b) # a |= b # python 3.9

		# {fullname:fullfolder} # exists_files
		pathbase_dict = {
			k: v for k, v in pathbase_dict.items() if os.path.exists(k)
		}
	...
	try:
		...
		if lfiles_sizes:
			...
			try:
				...
				lfiles = tmp[::-1]  # sorted(tmp, reverse=True)  # sort_by_string
	...
	for lf in filter(lambda x: x, tuple(lfiles)):
		...
		if fext != "mp4":
			...
			print(Style.BRIGHT + Fore.WHITE + "%s" % ";".join(
				[
					ofilename, project_file, source_file
				])
			)  # full_setup # is_need_json
		...
		try:
			...
			logging.warning("[hd] error %s" % ";".join(
				[
					lf, str(e)
				])
			)
		...
		try:
			...
			logging.warning("[sd] error %s" % ";".join(
				[
					lf,
					str(e)
				])
		...
		if vbr_update:
			vbr_dict |= vbr_update  # python 3.9
			# vbr_dict.update(vbr_update) 
			# vbr_dict_new = {**vbr_dict, **vbr_update} # a.update(b) # a |= b # python 3.9

		vbr_dict = {
			k: v for k, v in vbr_dict.items() if os.path.exists(k)
		}  # exists
		...
		***
		...
		try:
			...
			elif all((not is_change, width <= 640)) and all(
				(
					vcodec.lower().strip() == "h264",
					acodec.lower().strip() == "aac",
					any(
	...
	if filecmdbase_dict:
		...
		if jobs_list:
			...
			if files_dict:
				...
				files_dict = {
					k: v for k, v in files_dict.items() if os.path.exists(k)
				}
		...
		for k, v in filecmdbase_dict.items():
			...
			if all((k, v)):
				...
				if (
					...
					s_p_l_set.add(";".join([str(s_c), str(p_c), str(l_c)]))  # discord
					is_new = True
		...
		for sf in tuple(sorted_files):
			...
			if all((not fname in unique, fname)):
				unique.add(fname)  # short_file(first)_by_set # discord
				full_list.add(sf)  # full_filename(first)_by_set # discord
		if full_list:
		...
		with open(
			"c:\\downloads\\mytemp\\jresize.cmd", "a", buffering=1
		) as jcf:  # buffering
			jcf.writelines(
				"%s\n" % cmd.strip()
		...
		if combine_filter:
			...
			if all(
				...
				filecmdbase_dict |= filecmdbase_dict_new  # python 3.9
				# filecmdbase_dict.update(filecmdbase_dict_new)
				# filecmdbase_dict_new = {**filecmdbase_dict, **filecmdbase_dict_new} # a.update(b) # a |= b  # python 3.9
		...
		if (
			...
			try:
				...
				cbf_dict = {
					k: v for k, v in cbf_dict.items() if os.path.exists(k)
				}
			finally:
			...
			try:
				if all((len(optimal_sort) > 0, optimal_sort[-1])):
					...
					try:
						oss = sorted(
							[int(o) for o in optimal_sort], reverse=False
						)
					except:
						oss = 0
					else:
			...
			if std_dict:  # filter_by_exists_files
				std_dict = {
					k: v for k, v in std_dict.items() if os.path.exists(k)
				}
			...
			for k, v in filecmdbase_dict.items():
				...
				if (
					...
					hours_set.add(dt.hour)  # {1,2,3,4} # set(no_dict) # discord
				...
				if os.path.exists(k):
					...
					try:
						...
						# time_list = [
						# time_file // i for i in range(60, 3660) if all((time_file // i > 0, i % 60 == 0))
						# ]
				...
				if all((k, k in [*filecmdbase_pos_dict])):
					...
					if all((not int(prc_pos) in prc_pos_set, prc_pos <= 100)):
						prc_pos_set.add(int(prc_pos))  # discord
	...
	try:
		...
		if lanmacs_copy:
			lanmacs |= lanmacs_copy  # python 3.9
			# lanmacs.update(lanmacs_copy)  
			# lanmacs_new = {**lanmacs, **lanmacs_copy} # a.update(b) # a |= b  # python 3.9
			...
			for al in another_list:
				...
				if all(((fsize, dsize)):
					...
					if all((any_file, mp4_file, move_file2)):
						print(Style.BRIGHT + Fore.YELLOW + "%s [%s]" % ("$".join(
							[
								any_file,
								mp4_file,
								move_file2]),
					...
					if any_file[0] >= mp4_file[0]:  # and any_file.split(".")[-1].lower() != mp4_file.split(".")[-1].lower():  # drive_letter(diff/equal)
						...
						try:
							...
							if all((dfile1, dfile2, dfile3)):
								print(Style.BRIGHT + Fore.BLUE + "%s" % "=-=>".join(
									[
										dfile1,
										dfile2,
										dfile3
						...
						# year_filter = [
							# k.strip() for k in [*fb_dict] if year_regex.findall(k)
						# ] # for k, _ in fb_dict.items()
						...
						try:
							fdates = [
								v.strip() for _, v in trends_dict.items()
							]
						except:
							fdates = []
						else:
	...
	try:
		...
		fb_dict = {
			k: v for k, v in fb_dict.items() if os.path.exists(k)
		}
	finally:
	...
	if all(
		...
		try:
			...
			logging.error("@sound_notify run error")
		else:
			logging.info("@sound_notify run ok")

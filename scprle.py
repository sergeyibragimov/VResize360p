	...
	def get_profile_and_level(self, filename):
		...
		logging.info(
			...
			% (
				...
				"x".join([map(str, [pl_list[0], pl_list[-1]])]),
			)
		)
...
def width_height_profile_level(filename: str = "", ffmpeg: bool = True):
	...
	def ar_calc(h, w, nw=640) -> tuple:
		...
		try:
			...
			logging.warning(
				"@ar_calc value is normal [%s]" % "x".join([map(str, [nw, ac])])
			)
		else:
			try:
	...
	if cmd and all((nw, nh)):  # all((nw, nh, p, l))
		# logging.info(
		# "filename: %s, width/height: %s, profile/level: %s, length: %s, cmd: %s" % (
		# filename, "x".join([map(str, [nw, nh])]), ";".join([map(str, [p, l])]), length, codecs, cmd)
		# ) # ffmpeg
		logging.info(
			"filename: %s, width/height: %s, length: %s, codecs: %s, cmd: %s"
			% (
				add.full_to_short(filename),
				"x".join([map(str, [nw, nh])]),
				length,
				codecs,
				cmd,
			)
...
def walk(dr: str = "", files_template: str = ""):  # docstring(description)
...
if __name__ == "__main__":  # skip_main(debug)
	...
	for sf in filter(
		...
		try:
			...
			logging.info(
				"@calc_download_time current: %s, time: %s" % (fn, ":".join([hhmm, ss]))
			)
	...
	if fcd:
		...
		short_list = list(
			set(
				[crop_filename_regex.sub("", k.split("\\")[-1]) for k, _ in fcd.items()]
			)
		)  # some_jobs
		short_list.sort()
